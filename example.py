...
sentence = 'esa frase me parece muy buena .'
print(sentence)
...
accum = 0.0
accum += model.BaseScore(state, "esa", state2)
accum += model.BaseScore(state2, "frase", state)
#score defaults to bos = True and eos = True.  Here we'll check without the end
#of sentence marker.  
assert (abs(accum - model.score("esa frase", eos = False)) < 1e-3)
accum += model.BaseScore(state, "</s>", state2)
assert (abs(accum - model.score("esa frase")) < 1e-3)
